...
describe("get_hours_for_period", () => {
  ...
  it("resolves a project pseudonym to real hours", () => {
    const validated = t.validate({ period: { kind: "month", month: "August" }, projectPseudonym: "Project 4" });
    expect(validated.success).toBe(true);
    if (!validated.success) return;
    const result = t.run(ctx, validated.data);
    expect(result.data).toMatchObject({ totalHours: 15 });
  });

  it("returns a no-such-project caveat for an unknown pseudonym, rather than a made-up answer", () => {
...
describe("get_project_health", () => {
  it("joins Desk and Skylight data for Project 4", () => {
...
describe("get_category_spend", () => {
  it("resolves a category name case-insensitively", () => {
...
describe("list_projects_and_boards", () => {
  it("lists every pseudonym so the model can check existence before answering", () => {
    const t = findTool("list_projects_and_boards")!;
    const validated = t.validate({});
    expect(validated.success).toBe(true);
    if (!validated.success) return;
    const result = t.run(ctx, validated.data) as { data: { projects: string[]; boards: string[] } };
